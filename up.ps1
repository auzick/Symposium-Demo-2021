...
dotnet sitecore login --cm https://cm.basicnextjsdemo.localhost/ --auth https://id.basicnextjsdemo.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Invoke-RestMethod "https://cm.basicnextjsdemo.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.basicnextjsdemo.localhost/sitecore/
Start-Process https://www.basicnextjsdemo.localhost/
